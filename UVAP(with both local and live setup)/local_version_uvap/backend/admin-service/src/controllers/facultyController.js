...
exports.getFacultyProfile = async (req, res, next) => {
  try {
    const profile = await FacultyProfile.findOne({ user: req.user.id }).populate('user', 'name email phone');
...
// @desc    Update faculty settings (availability, etc.)
...
exports.getAllFaculty = async (req, res, next) => {
  try {
    ...
    // 2. Keyword Search (Name, Email, Department)
    if (req.query.keyword) {
      ...
      // Add to query: (Status matches) AND ((User is in userIds) OR (Department matches keyword))
      query = {
        ...query,
        $or: [
          { user: { $in: userIds } },
    ...
    // 3. Department Filter (if specific department selected)
    if (req.query.department && req.query.department !== 'All') {
      ...
      // Create reverse mapping (Short -> Full)
      const reverseDeptMapping = Object.entries(deptMapping).reduce((acc, [key, val]) => {
        acc[val] = key;
        return acc;
      }, {});

      const inputDept = req.query.department;
      const searchTerms = [inputDept];

      // If input is a full name, add its short code
      if (deptMapping[inputDept]) {
        searchTerms.push(deptMapping[inputDept]);
      }
      
      // If input is a short code, add its full name
      if (reverseDeptMapping[inputDept]) {
        searchTerms.push(reverseDeptMapping[inputDept]);
      }
      
      // Construct Regex to match ANY of the terms: ^(Term1|Term2)$
      const regexPattern = searchTerms.map(term => 
        term.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') // Escape regex special chars
      ).join('|');

      query.department = { $regex: `^(${regexPattern})$`, $options: 'i' };
    }
    ...
    const facultyWithWorkload = faculty.map(f => {
      const facObj = f.toObject();
      facObj.workload = workloadMap[f.user?._id.toString()] || 0;
      return facObj;
    });
    
    res.status(200).json({ success: true, count: facultyWithWorkload.length, data: facultyWithWorkload });
  } catch (err) {
    next(err);
  }
};
...
exports.deleteFaculty = async (req, res, next) => {
  try {
    ...
    const allocations = await CourseAllocation.find({ teacher: userId })
      .populate('course');

    // 2. Reassign each course
    const allocationService = require('../services/allocationService');
    const reassignments = [];

    for (const allocation of allocations) {
      // Find best replacement, excluding the current teacher
      const bestTeacher = await allocationService.findBestTeacher(
        allocation.course, 
        allocation.batch, 
        userId // Exclude current teacher
      );

      if (bestTeacher) {
        allocation.teacher = bestTeacher.user;
        reassignments.push(`Reassigned ${allocation.course.title} (${allocation.section}) to ${bestTeacher.user.name}`);
      } else {
        allocation.teacher = null; // Mark as Unassigned
        reassignments.push(`Marked ${allocation.course.title} (${allocation.section}) as Unassigned`);
      }
    ...
    res.status(200).json({ 
      success: true, 
      data: {}, 
      message: `Faculty deleted. ${reassignments.length} courses processed.`,
      reassignments 
    });
  } catch (err) {
    next(err);
...
exports.createFaculty = async (req, res, next) => {
  try {
    ...
    try {
      ...
      await sendEmail({
        to: personalEmail, // Send to personal email
...
// @desc    Get logged-in faculty's assigned courses
